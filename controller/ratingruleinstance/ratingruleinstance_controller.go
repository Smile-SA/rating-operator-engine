...
import (
	...
	ratingv1 "github.com/rating-operator-engine/apis/rating/v1"
...
// The ModelWatcher struct is the mechanism to handle each RatingRuleInstances resources.
// stopChan and configChan are used for communication and configuration updates, in case of changes by the user.
type ModelWatcher struct {
	// Holds the configuration for the RatingRuleInstance
	RatingRuleInstance ratingv1.RatingRuleInstanceSpec

	Name   string
	Logger logr.Logger

	stopChan   chan struct{}
	configChan chan ratingv1.RatingRuleInstanceSpec
...
// The ModelWatcher.timeframe function is a helper to generate a duration from the "timeframe" parameter of the RatingRuleInstance.
...
func (mw *ModelWatcher) query(ctx context.Context) *ModelWatcher {
	...
	cpu := mw.RatingRuleInstance.Cpu
	memory := mw.RatingRuleInstance.Memory
	price := mw.RatingRuleInstance.Price

	query := formatStr(mw.RatingRuleInstance.Metric, "{cpu}", cpu, "{memory}", memory, "{price}", price)
...
func (mw *ModelWatcher) convert(ctx context.Context) *ModelWatcher {
	...
	for _, result := range mw.UntreatedFrames.Data.Result {
		frame := metricToFrame(mw.RatingRuleInstance.Name, result.Metric)
		values := []float64{}
...
// replace string place holder with a value
func formatStr(format string, args ...string) string {
...
func (mw *ModelWatcher) send(ctx context.Context) error {
	...
	response, err := http.PostForm(addr, url.Values{
		...
		"metric":       []string{mw.RatingRuleInstance.Name},
...
// The main logging object, that is used to generate sub-loggers for each RatingRuleInstances.
var log = logf.Log.WithName("controller_ratingruleinstance")

// Add creates a new RatingRuleInstance Controller and adds it to the Manager. The Manager will set fields on the Controller
...
func newReconciler(mgr manager.Manager) reconcile.Reconciler {
	return &ReconcileRatingRuleInstance{client: mgr.GetClient(), scheme: mgr.GetScheme()}
}

// add adds a new Controller to mgr with r as the reconcile.Reconciler
func add(mgr manager.Manager, r reconcile.Reconciler) error {
	// Create a new controller
	c, err := controller.New("ratingruleinstance-controller", mgr, controller.Options{Reconciler: r})
	if err != nil {
		return err
	}

	// Watch for changes to primary resource RatingRuleInstance
	err = c.Watch(&source.Kind{Type: &ratingv1.RatingRuleInstance{}}, &handler.EnqueueRequestForObject{})
	if err != nil {
		return err
	}
...
// blank assignment to verify that ReconcileRatingRuleInstance implements reconcile.Reconciler
var _ reconcile.Reconciler = &ReconcileRatingRuleInstance{}

// ReconcileRatingRuleInstance reconciles a RatingRuleInstance object
type ReconcileRatingRuleInstance struct {
...
// Reconcile reads that state of the cluster for a RatingRuleInstance object and makes changes based on the state read
// and what is in the RatingRuleInstance.Spec
func (r *ReconcileRatingRuleInstance) Reconcile(request reconcile.Request) (reconcile.Result, error) {
	reqLogger := log.WithValues("Request.Namespace", request.Namespace, "Request.Name", request.Name)
	reqLogger.Info("Reconciling RatingRuleInstance")

	ctx := context.Background()
	// Fetch the RatingRuleInstance instance
	instance := &ratingv1.RatingRuleInstance{}
	...
	if exist {
		...
		watcher := &ModelWatcher{
			...
			configChan:  make(chan ratingv1.RatingRuleInstanceSpec),
